...
public sealed class CarUcCreateIt : TestBase, IAsyncLifetime {
   ...
   private ICarRepository _repo = null!;
   ...
   public async Task ExecuteAsync_MultipleCars_ShouldPersistAll() {
      ...
      var allCars = await _repo.SelectByAsync(null, null, CancellationToken.None);
      Assert.True(allCars.Count >= 5); // 3 Seed + 2 new

      var compactCars = await _repo.SelectByAsync(CarCategory.Compact, null, CancellationToken.None);
      Assert.True(compactCars.Count >= 1); // At least MULTI-101

      var availableCars = await _repo.SelectByAsync(null, CarStatus.Available, CancellationToken.None);
      Assert.True(availableCars.Count >= 5); // All created cars should be available

      var compactAvailable = await _repo.SelectByAsync(
         CarCategory.Compact,
         CarStatus.Available,
   ...
   public async Task ExecuteAsync_WithAllValidCategories_ShouldSucceed(CarCategory category) {
      ...
      var categoryCars = await _repo.SelectByAsync(category, null, CancellationToken.None);
