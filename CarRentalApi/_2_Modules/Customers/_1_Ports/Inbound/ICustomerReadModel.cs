...
public interface ICustomerReadModel {
   ...
   /// </summary>
   Task<Result<CustomerDetailDto>> FindByIdAsync(
      Guid Id,
   ...
   ///
   /// Returns:
   /// - Success with <see cref="CustomerDetailDto"/> if found
   /// - Failure if no customer with the given email exists
   /// </summary>
   Task<Result<CustomerDetailDto>> FindByEmailAsync(
      string email,
      CancellationToken ct
   );
   ...
   /// </summary>
   Task<Result<PagedResult<CustomerListItemDto>>> FilterAsync(
